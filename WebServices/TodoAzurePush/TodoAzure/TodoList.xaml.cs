...
namespace TodoAzure
{
    public partial class TodoList : ContentPage
    {
        ...
        protected override async void OnAppearing()
        {
            ...
            await RefreshItemsAsync(true, syncItems: false);
        }

        // Data methods
        async Task AddItemAsync(TodoItem item)
        {
            await manager.SaveTaskAsync(item);
            todoList.ItemsSource = await manager.GetTodoItemsAsync();
        }

        async Task CompleteItemAsync(TodoItem item)
        {
            item.Done = true;
            await manager.SaveTaskAsync(item);
            todoList.ItemsSource = await manager.GetTodoItemsAsync();
        ...
        public async void OnRefresh(object sender, EventArgs e)
        {
            ...
            try
            {
                await RefreshItemsAsync(false, true);
            }
            catch (Exception ex)
            {
                error = ex;
