    ...
    public class AuthenticationProvider : IAuthenticate
    {
        ...
        public async Task<bool> AuthenticateAsync()
        {
            ...
            try
            {
                if (user == null)
                {
                    user = await TodoItemManager.DefaultManager.CurrentClient.LoginAsync(MainActivity.Instance, MobileServiceAuthenticationProvider.Google, Constants.URLScheme);
                    if (user != null)
                    {
